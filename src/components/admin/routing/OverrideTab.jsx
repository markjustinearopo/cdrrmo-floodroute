...
/**
 * Routing → Override (was the Override Routes page).
 *
 * Take a saved route and replace its path — normally to detour around roads
 * flagged Flooded/Closed on Road Status, which are overlaid here as live
 * hazards. The planned route shows as a dashed ghost; the override draws as a
 * solid gold line, and the operator picks which version is active for dispatch.
 *
 * The automatic version of this detour is the same solve the Generate tab runs
 * with the flagged roads excluded — "Auto-detour" below calls it. What this
 * tab uniquely provides is the HAND-DRAWN override and the planned/override
 * active-version switch, which no other tab offers.
 */
export default function OverrideTab({ shared, onToast, onGoToTab }) {
  const { roads, graph, live } = shared
  ...
  function addPoint(latlng) {
    if (!selected) return onToast('Select a route to override first.')
  ...
  /**
   * Auto-detour: regenerate this route's path along the road network while
   * strongly avoiding the roads flagged Flooded/Closed and the flood-prone
   * areas in the live risk field. Routes through the planned route's own
   * waypoints so the override still visits the same key points.
   */
  function autoDetour() {
    if (!selected) return onToast('Select a route to re-route first.')
    if (!graph || graph.size === 0) return onToast('Road network unavailable — try again shortly.')
  ...
  function saveOverride() {
    if (!selected) return
    if (override.length < 2) return onToast('Draw at least two points for the override.')
    updateRoute(selected.id, { override, overrideAuto, active: 'override' })
    onToast(overrideAuto ? 'Auto-detour saved & set active.' : 'Override saved & set active.')
  }
  function clearOverride() {
    ...
    onToast('Reverted to planned route.')
  }
  ...
  return (
    <div className="override-routes rt-pane">
    <div className="ov-body">
      ...
      <aside className="ov-panel">
        {/* Route picker — mount point 3 of the one canonical list */}
        <section className="ov-section">
          ...
          <SavedRouteList
            ...
            selectHint="Override this route"
            emptyText={
              <>
                No saved routes yet. Create one on the{' '}
                <button type="button" className="ov-inline-link" onClick={() => onGoToTab('generate')}>Generate</button>
                {' '}or{' '}
                <button type="button" className="ov-inline-link" onClick={() => onGoToTab('draw')}>Draw</button>
                {' '}tab first.
              </>
            }
          />
        ...
        {selected && (
          <>
            {/* Hazard overlay + auto-detour */}
            <section className="ov-section">
              <h3 className="ov-section-title">Detour</h3>
              ...
              <button
                ...
                onClick={autoDetour}
                title="Auto-detour around flooded / closed roads (OpenStreetMap · Open-Meteo)"
              >
                <SparkIcon /> Auto-detour
              </button>
            </section>
            ...
            <section className="ov-section ov-section--grow">
              ...
              {override.length === 0 ? (
                <div className="ov-empty">
                  ...
                  <b> Auto-detour</b> to draw one automatically.
                </div>
              ) : overrideAuto ? (
                <div className="ov-empty">
